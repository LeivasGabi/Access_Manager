sap.ui.getCore().attachInit(function () {
  ...
  const pageContent: sap.ui.core.Control[] = [];

  type Student = { name: string; frequency: number; score: number };
  type Teacher = { name: string; student: Student[] };
  ...
  function buildTeacherPanel(p: Teacher): sap.m.Panel {
    const table = new sap.m.Table({
      inset: false,
      columns: [
        new sap.m.Column({ header: new sap.m.Label({ text: "Student" }) }),
        new sap.m.Column({ header: new sap.m.Label({ text: "Frequency (%)" }), hAlign: "Center" as any }),
        new sap.m.Column({ header: new sap.m.Label({ text: "Score" }), hAlign: "Center" as any })
      ]
    });
