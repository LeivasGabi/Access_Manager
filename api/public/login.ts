...
declare function setSession(user: User): void;

if (isLoggedIn()) {
  window.location.href = "/director.html";
}

sap.ui.getCore().attachInit(function (): void {
  const title = new sap.m.Title({ text: "School Portal" }).addStyleClass("sapUiTinyMarginBottom");

  const inpUser = new sap.m.Input({
    placeholder: "User",
    width: "100%",
    liveChange: (e) => (e.getSource() as sap.m.Input).setValueState(sap.ui.core.ValueState.None)
  });
  ...
  const makeToken = (u: string) => btoa(`${u}:${Date.now()}`);
